...
const SELECT_COLS =
  'id, tenant_id, service_id, day_of_week, start_time, end_time, created_at';
...
export function availabilityRulesRouter(pool: Pool): Router {
  ...
  router.post('/', async (req, res) => {
    const { tenantId, serviceId } = req.params as { tenantId: string; serviceId: string };
  ...
  router.get('/:id', async (req, res) => {
    const { tenantId, serviceId, id } = req.params as { tenantId: string; serviceId: string; id: string };
    if (req.auth!.tenantId !== tenantId) {
      res.status(403).json({ error: 'forbidden' });
      return;
    }
  ...
  router.patch('/:id', async (req, res) => {
    ...
    const result = await withTenantContext(pool, tenantId, async (client) => {
      ...
      if (await hasOverlap(client, serviceId, merged.dayOfWeek, merged.startTime, merged.endTime, id)) {

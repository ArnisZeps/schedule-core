...
- [x] Add `className="flex"` to the date `Input` in `NewAppointmentPanel` — overrides Safari iOS `display: inline-flex` (ignores `width: 100%`) with `display: flex` so the input respects the container width

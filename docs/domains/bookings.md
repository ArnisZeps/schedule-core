...
| File | Responsibility |
|------|----------------|
| `apps/web/src/components/calendar/NewAppointmentPanel.tsx` | Full-width on mobile, 384 px (`md:w-96`) on wider viewports. Accepts `mode?: 'new' \| 'reschedule'` and `rescheduleBooking?: Booking`. In `'new'` mode: service chips, staff dropdown, location selector, date + slot grid, override checkbox, conflict warning, notes, client fields. In `'reschedule'` mode: client/service/location/staff fields pre-filled and locked; submits via `useRescheduleBooking`. Both modes: "Custom time" collapsible section — start time picked via `TimeSelect` (the same 24h/5-min-step dropdown pair used in staff schedule), computed end time, sends `override: true`. Expanding the section seeds the dropdowns: the reschedule booking's start time, else the selected slot/prefill time, else `09:00` fallback — snapped to the nearest 5-minute option. The date input (`type="date"`) relies on a global base rule in `apps/web/app/globals.css` that sets `appearance: none` (+ `min-height` to restore the collapsed iOS height) on native date/time inputs — this releases iOS Safari's intrinsic minimum width so the field respects `w-full`/`min-w-0`. No per-input wrapper is needed. |

...
- As a business owner, I want to see and manage all my resources (staff, rooms) so that I can control what clients can book.
- As a business owner, I want to set weekly availability windows per resource so that bookings are only accepted during working hours.
...
- [ ] After successful login the user is redirected to /resources (or the originally requested path if stored).
...
- [ ] Sidebar shows at minimum: Resources link.
- [ ] On mobile viewports (< 768 px) the sidebar is hidden by default and toggled via a hamburger button in the header.

### Resources
- [ ] /resources lists all resources for the authenticated tenant with name and description.
- [ ] Empty state shown when no resources exist.
- [ ] "New resource" navigates to a create form.
- [ ] Submitting the create form adds the resource and returns to the list.
- [ ] Clicking a resource navigates to an edit form pre-populated with current values.
- [ ] Submitting the edit form updates the resource in place.
- [ ] Delete prompts confirmation then removes the resource from the list.
- [ ] Attempting to delete a resource that has bookings shows the server error inline.

### Availability rules
- [ ] Each resource has an /availability sub-page reachable from its edit form.

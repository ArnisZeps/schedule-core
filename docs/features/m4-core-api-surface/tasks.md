...
- [x] Integration tests: resource CRUD — incl. has_bookings block, cross-tenant 404 → `src/routes/resources.test.ts`
...
- [x] `apps/api/src/routes/resources.ts` — full CRUD under `/tenants/:tenantId/resources`
- [x] `apps/api/src/routes/availability-rules.ts` — full CRUD under `/tenants/:tenantId/resources/:resourceId/availability-rules`; overlap check helper
...
- [x] `resources.ts` GET /:id: added `AND tenant_id = $2` to WHERE clause for explicit cross-tenant isolation

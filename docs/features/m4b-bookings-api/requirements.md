...
- As a business owner, I want to filter bookings by service so that I can see one staff member's schedule.
...
- As a client, I want to see available time slots for a service on a given date so that I can pick a convenient time.
...
- [ ] Supports `?serviceId=UUID` to filter to a single service.
...
- [ ] Requires `serviceId`, `clientName`, `clientEmail`, `startAt`, `endAt`.
- [ ] Returns 409 if the slot overlaps an existing non-cancelled booking for the same service.
- [ ] Returns 409 if the slot falls outside the service's availability rules.
...
- [ ] `GET /public/:tenantSlug/services/:serviceId/slots?date=YYYY-MM-DD&duration=minutes` returns available slots.
